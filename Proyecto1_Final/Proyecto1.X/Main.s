...
accepted_reset:
    ...
    start_over:
    movlw   50
    movwf   reset_delay			    ;restart the reset delay
    movwf   seconds_delay		    ;restart the seconds delay
    bcf	    banderas_misc, changes_accepted
    clrf    banderas_semaforo
    bsf	    banderas_semaforo, green0_flag  ;reiniciar desde vía 1
    movlw   0b10001111
    andwf   banderas_misc		    ;asegurar que no esté blinking
    return
reject:
    movf    tiempo0, 0
    movwf   tiempo0_temp
